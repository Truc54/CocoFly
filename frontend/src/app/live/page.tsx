...
export default function LiveAuctionsPage() {
  ...
  }, [urlCategoryId, urlSearch, urlSort]);
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">
          {searchTerm
            ...
            : activeCategoryName
            ? `Đấu giá — ${activeCategoryName}`
            : "Đấu giá đang diễn ra"}
        </h1>
        {!loading && pagination && (
          <p className="text-sm text-slate-500 mt-1">{pagination.totalItems} phiên đang hoạt động</p>
        )}
      </div>
          ...
          {!loading && !error && auctions.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-lg font-bold text-slate-700 dark:text-slate-300 mb-2">
                {searchTerm ? `Không tìm thấy kết quả cho "${searchTerm}"` : "Chưa có phiên đấu giá nào"}
              </p>
              <p className="text-slate-500">
                {searchTerm ? "Hãy thử tìm kiếm với từ khóa khác" : "Hiện tại chưa có phiên đấu giá nào đang diễn ra. Hãy quay lại sau!"}
              </p>
            </div>
          )}
                ...
                {auctions.map((auction, idx) => (
                  ...
                  >
                    ...
                    <div className="p-3 space-y-2">
                      ...
                      <div className="w-full py-2 bg-primary text-white text-center text-xs font-bold rounded-lg mt-1 hover:bg-primary/90 active:scale-[0.98] transition-all cursor-pointer">
                        Đặt giá ngay
                      </div>
                    </div>
                  </Link>
