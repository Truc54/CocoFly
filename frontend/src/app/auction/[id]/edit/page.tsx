...
import { CustomSelect, CustomDateTimePicker } from "@/app/create-auction/custom-inputs";
...
export default function EditAuctionPage() {
      ...
      try {
        ...
        if (res.success) {
          ...
          if (auction.status !== "scheduled") {
             setFetchError("Chỉ có thể chỉnh sửa các phiên đấu giá sắp diễn ra.");
          ...
          setForm({
            ...
            scheduledStart: new Date(auction.scheduledStart).toISOString().slice(0, 16),
          ...
          setMaxStepReached(3); // Allow free navigation since it's an edit
        }
      } catch (err: any) {
        setFetchError(err.message || "Không thể tải thông tin phiên đấu giá.");
      } finally {
        setIsFetchingData(false);
      }
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setTimeout(() => setIsClosingToast(true), 700);
      setTimeout(() => {
      ...
      setSubmitError(err.message || "Cập nhật đấu giá thất bại. Vui lòng thử lại.");
    } finally {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-light dark:bg-background-dark">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 font-medium">Đang tải dữ liệu phiên đấu giá...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-light dark:bg-background-dark">
        <div className="flex flex-col items-center gap-4 max-w-sm text-center">
          ...
          <h3 className="text-xl font-bold text-slate-800 dark:text-white">Không thể chỉnh sửa</h3>
          <p className="text-slate-500">{fetchError}</p>
          <Button onClick={() => router.push('/manage-auctions')} className="mt-4 px-6 bg-primary text-white">
            Quay lại quản lý
          </Button>
        </div>
      </div>
          ...
          <div className={`fixed top-24 right-6 z-50 transition-all duration-300 ${
            ...
            <div className="bg-white dark:bg-slate-900 border-2 border-emerald-500 shadow-[4px_4px_0px_#059669] p-4 flex items-center gap-3 w-80">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-bold text-slate-900 dark:text-white truncate">Chỉnh sửa thành công!</h4>
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">Phiên đấu giá đã được cập nhật.</p>
              </div>
              <button 
                onClick={() => {
            ...
            {step < 3 ? (
              ...
              >
                {isSubmitting ? (
                  ...
                  <><Gavel className="w-4 h-4 mr-2" /> Cập nhật</>
                )}
              </Button>
            )}
          </div>
