...
**CocoFly** là một nền tảng đấu giá trực tuyến theo thời gian thực (Real-time Auction Platform) dành cho các sản phẩm đã qua sử dụng, đồ sưu tầm và các mặt hàng giá trị tại Việt Nam. Dự án được thiết kế và xây dựng theo mô hình kiến trúc phân lớp chuyên nghiệp, đáp ứng các tiêu chuẩn bảo mật và hiệu năng cao để sẵn sàng deploy lên môi trường Production.
...
### 1. Đấu giá Thời gian thực (Real-time Bidding)
...
* **Tự động gia hạn phút chót (Anti-Sniping):** Tự động kéo dài thời gian kết thúc đấu giá thêm 5 phút nếu có lượt đặt giá hợp lệ ở phút cuối cùng để đảm bảo tính công bằng.
* **Đặt giá tự động (Proxy Bidding):** Người mua thiết lập mức giá tối đa, hệ thống tự động trả giá tối thiểu cần thiết để giữ vị trí dẫn đầu.
* **Chống đặt giá trùng lặp (Idempotency Guard):** Ngăn chặn việc gửi liên tiếp các yêu cầu đặt giá trùng nhau thông qua Redis Lock.

### 2. Nhắn tin & Trao đổi Trực tiếp (Real-time Chat & DM)
* **Phòng chat sản phẩm (Auction Room Chat):** Nơi những người tham gia cùng bàn luận công khai về sản phẩm đang đấu giá.
* **Nhắn tin trực tiếp (Direct Message - DM):** Kênh liên lạc riêng tư giữa Người mua và Người bán hỗ trợ gửi ảnh, thu hồi tin nhắn và thả icon cảm xúc (Reactions).

### 3. Hệ thống Cảnh báo & Thông báo (Notifications)
* Tự động gửi thông báo trực quan (Real-time Toast) khi: bị vượt giá (outbid), đấu giá thành công, nhận tin nhắn mới hoặc yêu cầu thanh toán.

### 4. Quản trị & Vận hành (Admin Control)
...
Cấu hình thư mục được tổ chức theo cấu trúc Layered Architecture (Kiến trúc phân lớp) chặt chẽ và chuyên nghiệp:
...
## 🛡️ Các Biện pháp Bảo mật đã Triển khai (Security Audit)
...
4. **Socket.IO Event Rate Limiting:**
   * Đặt giá đấu giá (`bid:place`) giới hạn `5 lượt đặt giá / 10 giây`.
   * Gửi tin nhắn (`chat:send`, `dm:send`) giới hạn `10 tin nhắn / 10 giây` để ngăn chặn bot spam chat làm sập socket server.
...
### 2. Deploy Backend (Express) lên **VPS**
* Cài đặt **PM2** để quản lý tiến trình Node.js chạy ngầm:
  ```bash
  npm run build
  pm2 start dist/server.js --name "cocofly-api"
  ```
* Sử dụng **Nginx** làm Reverse Proxy để chuyển hướng request từ cổng `80/443` vào Node.js API (cổng `8000`), đồng thời cấu hình chứng chỉ SSL miễn phí từ Let's Encrypt.
* **Cấu hình Nginx Proxy Headers:** Cần thiết lập `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;` để Express middleware nhận diện IP chính xác của client đằng sau proxy.
