...
| AUC-04 | Cấu hình auction | startingPrice, reservePrice, buyoutPrice, bidIncrement, thời gian | Seller | **Must** |
...
| AE-03 | **Reserve Price** | Giá tối thiểu seller chấp nhận bán. Nếu `finalPrice < reservePrice` → auction `failed`. Reserve price ẩn với buyer |
...
| DA-03 | **Giá sàn** | Nếu giảm đến `reservePrice` mà không ai accept → `failed` |
...
stateDiagram-v2
    ...
    active --> ended : endTime reached + có bid ≥ reservePrice
    active --> failed : endTime reached + không bid / < reservePrice
